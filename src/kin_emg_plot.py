...
def plt_kin(kin_data, dh_data, plane, toe_off):
    ...
    for i in range(num_subplots):
        ...
        axs[i].fill_between(x_value, dh_data_tl, dh_data_dl, color = 'darkgrey')
        
        # Plotting right and left side                            _data

        axs[i].plot(x_value, data2plot[data2plot.columns[0]], color = 'forestgreen')
        axs[i].plot(x_value, data2plot[data2plot.columns[1]], color = 'red')
...
def plt_emg(emg_data, t_stance, e_hs):
    ...
    r_emg = emg_data.iloc[itr: ftr + 1, 1:5]
    l_emg = emg_data.iloc[itl: ftl + 1, 5:]
...
def data2table(plane, i, mm, rng):
    ...
    if plane != 'sagittal' or i == 0:
        ...
        data_mm = [
                [mm.index.tolist()[idx] for idx in [4, 6, 2, 12, 14, 16]],
        ...
        data_mm = [
                [mm.index.tolist()[idx] for idx in [4, 6, 12, 14, 16]],
