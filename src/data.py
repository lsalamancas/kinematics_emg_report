...
def read_data(age: int, p_path: str, run_selected: int) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if age < 14:
        ...
        sheet_name = 'DH_Adults' if input('Do you want to use adult Normative? Y/N: ').lower() == 'y' else 'DH_Children'
    else:
        sheet_name = 'DH_Adults'
    dh = pd.read_excel(os.sep.join([os.path.dirname(p_path),'normatives','dh_normal.xlsx']), sheet_name = sheet_name,  header = 7, index_col = 0).dropna()
